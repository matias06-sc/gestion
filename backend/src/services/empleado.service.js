  ...
  try {
    await run(
      `INSERT INTO empleados (
        ...
        cargoId, areaId, salario, fechaIngreso, estado, fotoPath, facialFotoPath, ownerAdminId, createdAt, updatedAt
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        id,
        nombres.trim(),
  ...
  try {
    await run(
      `UPDATE empleados SET
        ...
        cargoId = ?, areaId = ?, salario = ?, fechaIngreso = ?, estado = ?, fotoPath = ?, facialFotoPath = ?, updatedAt = ?
      WHERE id = ?`,
      [
        merged.nombres,
        merged.apellidos,
